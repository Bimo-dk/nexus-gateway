...
pub fn read_env() -> Result<Env> {
    let nexus_token = std::env::var("NEXUS_TOKEN")
        .context("NEXUS_TOKEN is required but not set")?;
    let registry_url = std::env::var("REGISTRY_URL")
        .context("REGISTRY_URL is required but not set")?;
    let gate_name = std::env::var("NEXUS_GATE_NAME")
        .context("NEXUS_GATE_NAME is required but not set")?;
...
async fn ensure_gate(client: &HyperClient, env: &Env) -> Result<()> {
    ...
    let host_id: Option<String> = if let Some(host_name) = env.host_name.as_deref() {
        let host_url = env.host_url.as_deref().ok_or_else(|| anyhow::anyhow!(
            "NEXUS_HOST_URL is required when NEXUS_HOST_NAME is set"
        ))?;
        ...
        let id = match http_client::get_json(
            ...
            Err(_) => {
                ...
                let remote_entry = env.host_remote_entry.as_deref().unwrap_or("/host/remoteEntry.json");
                ...
                ).await.context("failed to create host in registry")?;
    ...
    match http_client::post_json(client, &format!("{}/api/gates", base), &env.nexus_token, body).await {
...
pub async fn bootstrap(env: &Env) -> Result<(GatewayState, RouteTable)> {
    ...
    let gate_json = match http_client::get_json(&client, &gate_url, &env.nexus_token).await {
        ...
        Err(e) if e.to_string().contains("404") => {
            ...
            fetch_with_retry(&client, &gate_url, &env.nexus_token)
                .await
                .with_context(|| format!("gate '{}' not available after registration", env.gate_name))?
        }
        Err(_) => {
            fetch_with_retry(&client, &gate_url, &env.nexus_token)
                .await
    ...
    let gate: RegistryGate = serde_json::from_value(gate_json)
        .context("failed to deserialise gate response")?;
    ...
    let reg_config: RegistryGatewayConfig = fetch_with_retry(&client, &config_url, &env.nexus_token)
        .await
    ...
    let (host_id, host_name, host_url, host_framework, host_remote_entry, host_exposed_module) =
        if let Some(host) = gate.host {
            (host.id, host.name, host.url, host.framework, host.remote_entry, host.exposed_module)
        } else {
            (String::new(), String::new(), String::new(), Default::default(), String::new(), String::new())
        };
...
pub fn build_route_table(state: &GatewayState, remotes: &[RegistryRemote]) -> RouteTable {
    ...
    table.upsert(
        "/host/",
        UpstreamTarget { upstream_url: state.host_url.clone(), enabled: true },
    );
    for remote in remotes {
        ...
        table.upsert(prefix, UpstreamTarget {
...
pub fn is_visible(remote: &RegistryRemote, host_id: &str) -> bool {
    remote.visibility == "global"
        || remote.visibility == format!("host:{}", host_id)
}
